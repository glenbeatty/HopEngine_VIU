...
int main()
{
    ...
    GLFWwindow* window = glfwCreateWindow(mode->height, mode->height, "LearnOpenGL", NULL, NULL);
    if (window == NULL)
    {
    ...
    std::string vertex_string = load_shader("shaders/vert.shader");
    const char* vertex_source = vertex_string.c_str();
    std::string fragment_string = load_shader("shaders/frag.shader");
